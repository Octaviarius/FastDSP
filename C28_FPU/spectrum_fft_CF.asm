...
; DESCRIPTION:
;
;    This function computes a complex FFT.  The input buffer not necessarily 
;    be aligned to a multiple of the FFT size.  If it is not aligned then the 
;    output buffer will yield invalid results.  If you do not wish to align 
;    the input buffer then use the fft_cf_convert function.  Using this function 
;    will reduce cycle performance of the algorithm.
;
; FUNCTIONS:
;
;    void fft_cf_convert (FFT_REAL *)
;
;    Where fft_cf_t is a structure defined as:
;
;	typedef struct {
;		float	*InPtr;
;		float	*OutPtr;
...
;===========================================================================
; Function: void fft_cf_convert(FFT_COMPLEX *cfft)
;===========================================================================
...
_FPU_CFFTF32vars1Stages1and2andBitReverse_casm:
		...
		; Initialize input and output pointers and do pointer swap for following stages:
		MOVL		XAR2,@_CFFTF32_vars_InPtr         ; fft.InPtr
		MOVL		XAR3,@_CFFTF32_vars_OutPtr
		MOVL		@_CFFTF32_vars_CurrentOutPtr,XAR2
		MOVL		@_CFFTF32_vars_CurrentInPtr,XAR3
		...
		MOV32		R0H,*-SP[I0]		; R0H = D0
		MOV32		R1H,*-SP[I1]		; R1H = D1
		MOV32		R2H,*-SP[I2]		; R2H = D2
		ADDF32		R7H,R0H,R2H			; R7H = (D0 + D2)
		||MOV32		R3H,*-SP[I3]				; R3H = D3
		ADDF32		R6H,R1H,R3H			; R6H = (D1 + D3)
		||MOV32		R4H,*-SP[I4]				; R4H = D4
		SUBF32		R0H,R0H,R2H			; R0H = (D0 - D2)
		||MOV32		R5H,*-SP[I5]				; R5H = D5
		SUBF32		R2H,R1H,R3H			; R2H = (D1 - D3)
		||MOV32		R1H,*-SP[I6]				; R1H = D6
		ADDF32		R3H,R4H,R1H			; R3H = (D4 + D6)
		SUBF32		R4H,R4H,R1H			; R4H = (D4 - D6)
		ADDF32		R1H,R7H,R3H			; R1H = (D0 + D2) + (D4 + D6)
		SUBF32		R7H,R7H,R3H			; R7H = (D0 + D2) - (D4 + D6)
		||MOV32		R3H,*-SP[I7]				; R3H = D7
		ADDF32		R1H,R5H,R3H			; R1H = (D5 + D7)
		||MOV32		*XAR3++,R1H
		SUBF32		R5H,R5H,R3H			; R5H = (D5 - D7)
		ADDF32		R3H,R6H,R1H			; R3H = (D1 + D3) + (D5 + D7)
